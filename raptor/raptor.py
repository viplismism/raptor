    ...
    def __init__(
        ...
        if tree_builder_config is None:
            ...
            raise ValueError(
                ...
                f"{tree_builder_config_class} for tree_builder_type '{tree_builder_type}'"
            )
...
class RetrievalAugmentation:
    ...
    def add_documents(self, docs, force=False):
        """
        Adds documents to the tree and creates a TreeRetriever instance.

        Args:
            docs: The input text to add to the tree.
            force: If True, overwrite existing tree without prompting.
        """
        if self.tree is not None and not force:
            logger.warning(
                "Overwriting existing tree. Pass force=True to suppress this warning."
            )
    ...
    def retrieve(
        ...
        if self.retriever is None:
            raise ValueError(
                "The TreeRetriever instance has not been initialized. "
